...
def executeCmd(msg, text, txt, cmd, msg_id, receiver, sender, to, setKey):
    if cmd == 'logoutbot':
        ...
        line.sendReplyMessage(msg_id, to, 'กำลังเริ่มระบบ ใหม่')
        restartProgram()
    elif cmd == 'help':
        line.sendReplyMessage(msg_id, to, help())
    elif cmd == 'speed':
        start = time.time()
        line.sendReplyMessage(msg_id, to, 'กำลังทดสอบ...')
        elapse = time.time() - start
        line.sendReplyMessage(msg_id, to, 'ความเร็วที่ใช้ส่งข้อความ.. %s วินาที' % str(elapse))
    elif text lower() == 'Me':
        ...
        line.sendReplyMessage(msg_id, to, 'เวลา Login ระบบ ' + format_timespan(runtime))
    elif cmd == 'author':
        ...
        if cmd == 'autorespondmention':
            ...
            if settings['autoRespondMention']['status']:
                line.sendReplyMessage(msg_id, to, 'เปิดใช้งานสำเร็จ...')
            else:
                settings['autoRespondMention']['status'] = True
                line.sendReplyMessage(msg_id, to, 'เปิดใช้งานอยู่แล้ว..')
        elif texttl == 'off':
            if not settings['autoRespondMention']['status']:
                line.sendReplyMessage(msg_id, to, 'ปิดใช้งานสำเร็จ...')
            else:
                settings['autoRespondMention']['status'] = False
                line.sendReplyMessage(msg_id, to, 'ปิดใช้งานอยู่แล้ว..')
        else:
            settings['autoRespondMention']['message'] = textt
            line.sendReplyMessage(msg_id, to, 'เปลี่ยนข้อความคนแท็กเป็น `%s`' % textt)
        ...
        if cmd == 'autorespond':
            ...
            line.sendReplyMessage(msg_id, to, 'เปลี่ยนข้อความตอบแชทเป็น `%s`' % textt)
